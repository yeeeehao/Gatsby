...
export default function EditPage(props) {
  ...
  const apiUrl =
    "https://sample-blog-express-78qzajv5h-mchayapol.vercel.app/api/blogs"
  ...
  React.useState(() => {
    fetch(apiUrl + "/" + params.get("id"))
      ...
      .then(doc => {
        console.log(doc)
        if (formRef.current.topic) formRef.current.topic.value = doc.topic
        if (formRef.current.content) formRef.current.content.value = doc.content
        if (formRef.current.author) formRef.current.author.value = doc.author
      })
  }, [])
